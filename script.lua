--[[
    BEDWARS SCRIPT - NEW STANDALONE UI
    Replaces the vape UI library while preserving ALL original game logic
    
    This creates a custom UI system that mimics the vape API so all the
    original module code works without modification.
]]
...
local Theme = {
    ...
    Enabled = Color3.fromRGB(138, 43, 226),
    ...
    WindowSize = UDim2.fromOffset(620, 450),
}
...
local function Tween(obj, props, duration)
    local t = TweenService:Create(obj, TweenInfo.new(duration or 0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), props)
    t:Play()
    return t
end
...
local function CreateNotification(title, message, duration, notifType)
    ...
    local colors = {
        ...
        alert = Color3.fromRGB(240, 71, 71),
    }
...
function Module.new(name, category, config)
    ...
    self.Keybind = ""
    self.ExtraText = config.ExtraText
    
    -- Cleanup system (CRITICAL for proper functionality)
    self._connections = {}
    self._cleanupFuncs = {}
    
    self.Children = nil -- Will hold sub-elements
    self.Object = nil   -- The UI frame
...
function Module:Toggle()
    ...
    if self.Callback then
        ...
        if not success then
            warn("[Module Error] " .. self.Name .. ": " .. tostring(err))
        end
    end
end
...
function Module:_updateVisual()
    if self._toggleBg then
        Tween(self._toggleBg, {BackgroundColor3 = self.Enabled and Theme.Enabled or Theme.Disabled}, 0.2)
    end
    if self._toggleCircle then
        Tween(self._toggleCircle, {Position = self.Enabled and UDim2.new(1, -17, 0.5, 0) or UDim2.new(0, 3, 0.5, 0)}, 0.2)
    end
    if self._nameLabel then
        Tween(self._nameLabel, {TextColor3 = self.Enabled and Theme.Text or Theme.TextDim}, 0.2)
    end
end

-- CRITICAL: Clean function that modules use to register cleanup items
function Module:Clean(item)
    if typeof(item) == "RBXScriptConnection" then
        table.insert(self._connections, item)
    elseif typeof(item) == "function" then
        table.insert(self._cleanupFuncs, item)
    elseif typeof(item) == "Instance" then
        table.insert(self._cleanupFuncs, function()
            if item and item.Parent then
                item:Destroy()
            end
        end)
    elseif typeof(item) == "table" and item.Disconnect then
        table.insert(self._connections, item)
    end
    return item
end

-- Run all cleanup when module is disabled
function Module:_runCleanup()
    ...
    for _, func in ipairs(self._cleanupFuncs) do
        local success, err = pcall(func, self)
        if not success then
            warn("[Cleanup Error] " .. self.Name .. ": " .. tostring(err))
        end
    end
    table.clear(self._cleanupFuncs)
end

function Module:SetBind(key)
    self.Keybind = key
    if self._keybindLabel then
        self._keybindLabel.Text = key ~= "" and "[" .. key .. "]" or ""
    end
end

--============================================================================
-- MODULE UI ELEMENT CREATORS
--============================================================================
...
function Module:CreateToggle(config)
    ...
    local frame = Create("Frame", {
        Name = config.Name,
        BackgroundColor3 = config.Darker and Theme.BackgroundTertiary or Theme.BackgroundSecondary,
        BackgroundTransparency = config.Darker and 0 or 1,
        Size = UDim2.new(1, 0, 0, 32),
        Parent = self.Children
    })
    if config.Darker then AddCorner(frame, 4) end
    ...
    local toggleBg = Create("Frame", {
        ...
        BackgroundColor3 = toggle.Enabled and Theme.Enabled or Theme.Disabled,
    ...
    local toggleCircle = Create("Frame", {
        ...
        Position = toggle.Enabled and UDim2.new(1, -17, 0.5, 0) or UDim2.new(0, 3, 0.5, 0),
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        Tween(toggleBg, {BackgroundColor3 = toggle.Enabled and Theme.Enabled or Theme.Disabled}, 0.2)
        Tween(toggleCircle, {Position = toggle.Enabled and UDim2.new(1, -17, 0.5, 0) or UDim2.new(0, 3, 0.5, 0)}, 0.2)
        
        if toggle.Function then
            task.spawn(function()
...
function Module:CreateSlider(config)
    ...
    local valueLabel = Create("TextLabel", {
        ...
        FontFace = Theme.Font,
        Text = tostring(slider.Value) .. getSuffix(slider.Value),
        TextColor3 = Theme.Text,
        TextSize = 12,
    ...
    local function update(input)
        ...
        val = math.floor(val * slider.Decimal + 0.5) / slider.Decimal
        
        slider.Value = val
        valueLabel.Text = tostring(val) .. getSuffix(val)
        sliderFill.Size = UDim2.new(pct, 0, 1, 0)
        
        if slider.Function then
            task.spawn(function()
                slider.Function(val)
            end)
        end
...
function Module:CreateDropdown(config)
    ...
    local function populateList()
        ...
        for _, item in ipairs(dropdown.List) do
            ...
            itemBtn.MouseButton1Click:Connect(function()
                ...
                dropdown.Open = false
                Tween(listFrame, {Size = UDim2.new(0.6, -8, 0, 0)}, 0.2)
    ...
    dropBtn.MouseButton1Click:Connect(function()
        ...
        if dropdown.Open then
            ...
            local h = math.min(#dropdown.List * 26 + 10, 200)
            Tween(listFrame, {Size = UDim2.new(0.6, -8, 0, h)}, 0.2)
        else
            Tween(listFrame, {Size = UDim2.new(0.6, -8, 0, 0)}, 0.2)
            task.delay(0.2, function()
                listFrame.Visible = false
...
function Module:CreateColorSlider(config)
    ...
    local function updateColor()
        ...
        if colorSlider.Function then
            task.spawn(function()
                colorSlider.Function(colorSlider.Hue, colorSlider.Sat, colorSlider.Value, colorSlider.Opacity)
            end)
        end
    end
    
    local draggingHue, draggingOpacity = false, false
    ...
    opacityBg.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
    ...
    UserInputService.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement then
            ...
            elseif draggingOpacity then
                local pct = math.clamp((input.Position.X - opacityBg.AbsolutePosition.X) / opacityBg.AbsoluteSize.X, 0, 1)
                colorSlider.Opacity = pct
                updateColor()
            end
        end
    end)
...
function Module:CreateTwoSlider(config)
    ...
    function slider.GetRandomValue()
        return math.random(math.floor(slider.ValueMin), math.floor(slider.ValueMax))
    end
    ...
    local minPct = (slider.ValueMin - slider.Min) / (slider.Max - slider.Min)
    local maxPct = (slider.ValueMax - slider.Min) / (slider.Max - slider.Min)
    
    local sliderFill = Create("Frame", {
        BackgroundColor3 = Theme.Accent,
        Position = UDim2.new(minPct, 0, 0, 0),
        Size = UDim2.new(maxPct - minPct, 0, 1, 0),
        Parent = sliderBg
    })
    AddCorner(sliderFill, 5)
    ...
    local function updateDisplay()
        ...
        local mp = (slider.ValueMin - slider.Min) / (slider.Max - slider.Min)
        local xp = (slider.ValueMax - slider.Min) / (slider.Max - slider.Min)
        sliderFill.Position = UDim2.new(mp, 0, 0, 0)
        sliderFill.Size = UDim2.new(xp - mp, 0, 1, 0)
    end
    ...
    UserInputService.InputChanged:Connect(function(input)
        if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
            ...
            if math.abs(val - slider.ValueMin) < math.abs(val - slider.ValueMax) then
                slider.ValueMin = math.min(val, slider.ValueMax)
            else
                slider.ValueMax = math.max(val, slider.ValueMin)
            end
            
            updateDisplay()
            if slider.Function then
                task.spawn(function()
                    slider.Function(slider.ValueMin, slider.ValueMax)
                end)
            end
        end
    end)
...
function Module:CreateTextBox(config)
    ...
    input.FocusLost:Connect(function()
        ...
        if textbox.Function then
            task.spawn(function()
                textbox.Function(textbox.Value)
            end)
        end
    end)
...
function Module:CreateTextList(config)
    local textlist = {
        Name = config.Name,
        List = config.Default and table.clone(config.Default) or {},
        ListEnabled = config.Default and table.clone(config.Default) or {},
        Function = config.Function,
        Object = nil,
    }
    ...
    input.FocusLost:Connect(function(enterPressed)
        if enterPressed and input.Text ~= "" then
            table.insert(textlist.ListEnabled, input.Text)
            table.insert(textlist.List, input.Text)
            listDisplay.Text = table.concat(textlist.ListEnabled, ", ")
            input.Text = ""
            if textlist.Function then
                task.spawn(function()
                    textlist.Function(textlist.ListEnabled)
                end)
            end
        end
    end)
...
function Module:CreateTargets(config)
    ...
    local xPos = 0.28
    for _, tType in ipairs({"Players", "NPCs", "Walls"}) do
        if config[tType] ~= nil then
            local btn = Create("TextButton", {
                BackgroundColor3 = targets[tType].Enabled and Theme.Accent or Theme.BackgroundTertiary,
                Position = UDim2.new(xPos, 0, 0.15, 0),
                Size = UDim2.new(0, 55, 0.7, 0),
                FontFace = Theme.Font,
                Text = tType:sub(1,3),
                TextColor3 = Theme.Text,
                TextSize = 10,
            ...
            btn.MouseButton1Click:Connect(function()
                targets[tType].Enabled = not targets[tType].Enabled
                Tween(btn, {BackgroundColor3 = targets[tType].Enabled and Theme.Accent or Theme.BackgroundTertiary}, 0.2)
                if targets.Function then
                    task.spawn(targets.Function)
                end
            end)
...
function Module:CreateFont(config)
    ...
    local fonts = {"Gotham", "SourceSans", "Arial", "Roboto", "Ubuntu"}
    local dropdown = self:CreateDropdown({
        Name = config.Name,
        List = fonts,
        Darker = config.Darker,
        Function = function(val)
            ...
            fontSel.Value = map[val] or map.Gotham
            if fontSel.Function then
                task.spawn(function()
                    fontSel.Function(fontSel.Value)
                end)
            end
        end
    })
    
    fontSel.Object = dropdown.Object
    return fontSel
end
...
function Category.new(name, parent, tabContainer, moduleContainer)
    ...
    self.Options = {}
    self.ListEnabled = {}
    self.ColorUpdate = Instance.new("BindableEvent")
    ...
    AddCorner(self.TabButton, 6)
    
    -- Create module container
    self.ModuleFrame = Create("ScrollingFrame", {
        Name = name .. "_Modules",
        BackgroundTransparency = 1,
        Size = UDim2.new(1, 0, 1, 0),
    ...
    Create("UIListLayout", {
        ...
        Parent = self.ModuleFrame
    })
    AddPadding(self.ModuleFrame, 6, 6, 6, 10)
    
    return self
end

function Category:CreateModule(config)
    local module = Module.new(config.Name, self, config)
    ...
    module._toggleCircle = toggleCircle
    ...
    module._keybindLabel = keybindLabel
    ...
    AddPadding(module.Children, 0, 8, 12, 12)
    ...
    expandBtn.MouseButton1Click:Connect(function()
        ...
        module.Children.Visible = expanded
        expandBtn.Text = expanded and "▲" or "▼"
    end)
    
    -- Hover effect
    headerBtn.MouseEnter:Connect(function()
        Tween(frame, {BackgroundColor3 = Theme.BackgroundTertiary}, 0.15)
    end)
    headerBtn.MouseLeave:Connect(function()
        Tween(frame, {BackgroundColor3 = Theme.BackgroundSecondary}, 0.15)
    end)
...
function Category:Select()
    ...
    Tween(self.TabButton, {BackgroundTransparency = 0, TextColor3 = Theme.Text}, 0.2)
end

function Category:Deselect()
    self.ModuleFrame.Visible = false
    Tween(self.TabButton, {BackgroundTransparency = 1, TextColor3 = Theme.TextMuted}, 0.2)
end

--============================================================================
-- MAIN UI (VAPE REPLACEMENT)
--============================================================================
local UI = {}
...
function UI:_createUI()
    ...
    self.ScreenGui = Create("ScreenGui", {
        Name = "BedwarsUI_" .. tostring(math.random(1000, 9999)),
        ResetOnSpawn = false,
    ...
    if not self.ScreenGui.Parent then
        self.ScreenGui.Parent = lplr:WaitForChild("PlayerGui")
    end
    ...
    closeBtn.MouseLeave:Connect(function() Tween(closeBtn, {BackgroundTransparency = 0.7}, 0.15) end)
    ...
    self._moduleArea = moduleArea
    
    -- Make draggable
    self:_makeDraggable(titleBar, self.MainFrame)
    
    -- Create categories
    local catOrder = {"Combat", "Blatant", "Render", "Utility", "World", "Inventory", "Legit", "Friends", "Targets"}
    for i, name in ipairs(catOrder) do
        local cat = Category.new(name, self, tabContainer, moduleArea)
        cat.TabButton.LayoutOrder = i
        self.Categories[name] = cat
        
        cat.TabButton.MouseButton1Click:Connect(function()
            self:_selectCategory(cat)
        end)
        
        cat.TabButton.MouseEnter:Connect(function()
            if self.SelectedCategory ~= cat then
                Tween(cat.TabButton, {BackgroundTransparency = 0.5}, 0.1)
            end
        end)
        cat.TabButton.MouseLeave:Connect(function()
            if self.SelectedCategory ~= cat then
                Tween(cat.TabButton, {BackgroundTransparency = 1}, 0.1)
            end
        end)
    end
    
    -- Alias for Legit category
    self.Legit = self.Categories.Legit
    
    -- Select first category
    self:_selectCategory(self.Categories.Combat)
end

function UI:_selectCategory(cat)
...
function UI:_makeDraggable(handle, frame)
    ...
    UserInputService.InputBegan:Connect(function(input, processed)
        ...
        for _, module in pairs(self.Modules) do
            if module.Keybind ~= "" then
                local success, keyCode = pcall(function() return Enum.KeyCode[module.Keybind] end)
...
function UI:ToggleVisibility()
    ...
    if self.Visible then
        ...
        Tween(self.MainFrame, {Size = Theme.WindowSize}, 0.3)
    else
...
function UI:CreateNotification(title, message, duration, notifType)
    return CreateNotification(title, message, duration, notifType)
end

function UI:Clean(item)
    if typeof(item) == "RBXScriptConnection" then
...
function UI:Uninject()
    ...
    for _, cat in pairs(self.Categories) do
        for _, mod in pairs(cat.Modules) do
            if mod.Enabled then
                mod:Toggle() -- Disable and cleanup
            end
        end
    end
...
--============================================================================
-- CREATE THE UI INSTANCE AND SET UP shared.vape COMPATIBILITY
--============================================================================
local vape = UI.new()
vape.Loaded = true

-- Store modules in main table too
setmetatable(vape.Modules, {
    __index = function(self, key)
        for _, cat in pairs(vape.Categories) do
            if cat.Modules[key] then
                return cat.Modules[key]
            end
        end
        return nil
    end
...
--============================================================================
-- SET UP LIBRARIES THAT ORIGINAL SCRIPT EXPECTS
--============================================================================

-- These libraries need to exist for the original game logic
vape.Libraries = vape.Libraries or {}

vape.Libraries.targetinfo = vape.Libraries.targetinfo or {Targets = {}}
...
vape.Libraries.getfontsize = vape.Libraries.getfontsize or function(text, size, font, bounds)
    local success, result = pcall(function()
        return TextService:GetTextSize(text, size, Enum.Font.Gotham, bounds)
    end)
    return success and result or Vector2.new(#text * size * 0.5, size)
end

vape.Libraries.getcustomasset = vape.Libraries.getcustomasset or function(path)
...
vape.Libraries.auraanims = {
    Normal = {{CFrame = CFrame.Angles(0, 0, math.rad(30)), Time = 0.12}, {CFrame = CFrame.Angles(0, 0, math.rad(-30)), Time = 0.12}},
    Spin = {{CFrame = CFrame.Angles(0, math.rad(360), 0), Time = 0.3}},
    Random = {{CFrame = CFrame.identity, Time = 0.1}}
}

--============================================================================
-- ENTITY LIBRARY (CRITICAL FOR GAME LOGIC)
--============================================================================
local EntityLibrary = {
    isAlive = false,
    character = nil,
...
-- Events using BindableEvents
EntityLibrary.Events = {
...
function EntityLibrary.getEntityColor(entity)
    if entity and entity.Friend then
        return Color3.fromRGB(0, 255, 255)
    elseif entity and entity.Target then
        return Color3.fromRGB(255, 0, 0)
    end
    return nil
end

function EntityLibrary.start()
    EntityLibrary.Running = true
end

function EntityLibrary.addEntity(char, plr, teamCheck)
    -- Will be overridden by the original game logic
end

function EntityLibrary.removeEntity(char, isLocal)
    -- Will be overridden by the original game logic
end

function EntityLibrary.refreshEntity(char, plr)
    -- Will be overridden by the original game logic
end

function EntityLibrary.addPlayer(plr)
    -- Will be overridden by the original game logic
end

function EntityLibrary.EntityPosition(config)
    if not EntityLibrary.isAlive then return nil end
    
    local closest, closestDist = nil, config.Range or math.huge
    local localPos = EntityLibrary.character and EntityLibrary.character.RootPart and EntityLibrary.character.RootPart.Position or Vector3.zero
    
    for _, entity in pairs(EntityLibrary.List) do
        if entity.Targetable and entity.RootPart then
...
function EntityLibrary.EntityMouse(config)
    if not EntityLibrary.isAlive then return nil end
    
    local camera = workspace.CurrentCamera
    local closest, closestAngle = nil, config.Range or 180
    local origin = config.Origin or (EntityLibrary.character and EntityLibrary.character.RootPart and EntityLibrary.character.RootPart.Position) or Vector3.zero
    
    for _, entity in pairs(EntityLibrary.List) do
        if entity.Targetable and entity.RootPart then
            local screenPos, onScreen = camera:WorldToScreenPoint(entity.RootPart.Position)
            if onScreen or not config.OnScreen then
                local mouse = UserInputService:GetMouseLocation()
                local entityScreenPos = Vector2.new(screenPos.X, screenPos.Y)
                local angle = (mouse - entityScreenPos).Magnitude
...
function EntityLibrary.AllPosition(config)
    ...
    -- Sort
    if config.Sort then
        table.sort(results, config.Sort)
    else
...
function EntityLibrary.targetCheck(ent)
    if ent.TeamCheck then
        return ent:TeamCheck()
    end
    if ent.NPC then return true end
    return ent.Targetable
end

vape.Libraries.entity = EntityLibrary

-- Make shared.vape point to our new UI
shared.vape = vape

CreateNotification("Bedwars", "UI Loaded! Press RightShift to toggle", 5, "success")

--============================================================================
-- ORIGINAL GAME LOGIC STARTS HERE  
-- Note: We removed 'return vape' so the script continues executing
--============================================================================

-- Additional services needed by original script
...
-- Re-reference vape and its libraries for the game logic below
